...
def test_login(client, setup_database):
    ...
    Test valid login
    """
    response = client.post('/api/login', json={'username': 'test', 'password': 'test'}).status_code
    assert response == 200
...
def test_bad_login(client):
    ...
    Test bad login
    """
    response = client.post('/api/login', json={'username': 'test', 'password': 'wrong'}).status_code
    assert response == 401
...
def test_addscore(client):
    ...
    Test add score
    """
    response = client.post('/api/add_score', json={'username': 'test', 'score': 10}).status_code
    assert response == 201
